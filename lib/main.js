...
var STACK, application, appsPath, async, axon, clients, cozyStack, db, exec, fs, helpers, humanize, log, logError, monitoring, path, pkg, program, request, rmdir, stackApplication, version,
...
program.command('remove-old-app-on-disk').description('Reinstall all user applications, usefull for cozy relocation').action(function() {
  var apps;
  apps = ["calendar", "contacts", "sync", "import-from-google", "emails", "files", "photos"];
  return async.forEachSeries(apps, function(app, next) {
    return rmdir("/usr/local/cozy/apps/" + app, next);
  }, function(err) {
      ...
      return async.forEachSeries(apps, function(app, next) {
        ...
        switch (app.state) {
          case 'installed':
            ...
            if (ref = app.slug, indexOf.call(specific_apps, ref) >= 0) {
              return application.reinstall(app.slug, app, callback);
            } else {
              return application.installController(app, callback);
            }
            break;
          case 'stopped':
            log.info((app.slug + " : stopped. Reinstall ") + "application if necessary and stop it...");
            return application.installController(app, function(err) {
              if (err != null) {
                return callback(err);
              }
              return application.stopController(app.slug, callback);
            });
          case 'installing':
